function New-Platform-Landing-Zone {
    <#
    .SYNOPSIS
    Deploys an accelerator according to the supplied inputs.
    .DESCRIPTION
    This function is used to create accelerators consisting or bootstrap and optionally starter modules. The accelerators are designed to simplify and speed up configuration of common Microsoft patterns, such as CI / CD for Azure Landing Zones.
    .EXAMPLE
    New-Platform-Landing-Zone
    .EXAMPLE
    New-Platform-Landing-Zone -c "./config.yaml" -o "."
    .EXAMPLE
    New-Platform-Landing-Zone -c "./config.yaml" -o "." -zones_support ([PSCustomObject]@{ region = "eastus"; zones = @("1", "2", "3") })
    #>
    ...
    param (
        ...
        [Parameter(
            Mandatory = $false,
            HelpMessage = "[OPTIONAL] The availability zones support configuration. Environment variable: ALZ_zones_support. Config file input: zones_support."
        )]
        [Alias("zs")]
    ...
    if ($PSCmdlet.ShouldProcess("Accelerator setup", "modify")) {
        ...
        $bootstrapAndStarterConfig = Get-BootstrapAndStarterConfig2 `
        ...
        $zonesSupport = if ($null -eq $zones_support) { @() } else { $zones_support }
